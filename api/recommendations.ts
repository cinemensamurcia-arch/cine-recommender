...
Tienes una lista de PELÍCULAS CANDIDATAS que vienen de las recomendaciones de TMDB
en base a otras películas que el usuario ha valorado muy bien.

TU TAREA:

- Elegir hasta N películas de esa lista de candidatas.
- Para cada una, generar una explicación en español, de varias frases, natural y humana,
  de por qué crees que le va a gustar al usuario.
- Debes tener en cuenta:
  - Qué tipo de historias disfruta (según sus notas).
  - Si valora más el guion, el disfrute, las actuaciones, la BSO, etc.
  - El tono emocional, el ritmo, el tipo de personajes, los temas, etc.
...
- No repitas la misma plantilla en todas las explicaciones.
- Varía el estilo: unas más centradas en emoción, otras en guion, otras en atmósfera, etc.
- NO menciones TMDB ni que vienen de una API.
- NO digas frases tipo "Te la recomiendo porque la valoraste con X/10".
- No expliques el proceso, solo habla al usuario de la película y de por qué podría conectarle.

Devuelve SIEMPRE JSON puro con el formato EXACTO:
...
Estas son algunas de sus valoraciones (para que veas qué le gusta):
...
Devuélveme hasta ${max} recomendaciones, seleccionadas SOLO de esa lista de candidatas,
con un "reason" distinto y humano para cada una.
`;
